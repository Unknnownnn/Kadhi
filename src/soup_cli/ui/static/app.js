...
async function loadDashboard() {
  try {
    const [runsResult, systemResult] = await Promise.allSettled([
    ...
    if (systemResult.status === 'fulfilled') {
      systemInfo = systemResult.value;
    } else {
      systemInfo = {
        device_name: 'System information unavailable',
        gpu_info: { memory_total: 'Unavailable' },
        python_version: '-',
        version: '-',
      };
    }
...
function renderDashboard() {
  ...
  document.getElementById('dashboard-content').innerHTML = `
    ...
    <div class="card">
      ...
      ${runsData.length === 0
        ? '<div class="empty-state"><div class="empty-state-text">No runs yet</div><div class="empty-state-hint">Start training with "kadhi train" or use New Training</div></div>'
