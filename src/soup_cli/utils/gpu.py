...
def get_gpu_info(backend: Optional[str] = None) -> dict:
    ...
    # Keep dashboard hardware detection useful when PyTorch was installed
    # without CUDA support. Reuse the monitor's bounded nvidia-smi probe.
    try:
        from soup_cli.utils.gpu_monitor import query_nvidia_smi

        smi_ok, samples = query_nvidia_smi()
        if smi_ok and samples:
            total_mb = sum(sample.mem_total_mb or 0.0 for sample in samples)
            return {
                "memory_total": f"{total_mb / 1024:.1f} GB (driver)",
                "memory_total_bytes": int(total_mb * 1024 * 1024),
                "gpu_count": len(samples),
                "gpu_names": [sample.name for sample in samples],
